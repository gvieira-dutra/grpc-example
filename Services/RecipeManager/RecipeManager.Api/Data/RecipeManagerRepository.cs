...
public sealed class RecipeManagerRepository(IDocumentSession session) : IRecipeManagerRepository
{
    ...
    public async Task<bool> DeleteAsync(string recipeName, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<Recipe> GetAsync(string recipeNmae, CancellationToken cancellationToken = default)
    {
        var recipe = await session.LoadAsync<Recipe>(recipeNmae, cancellationToken);

        return recipe is null ? throw new RecipeNotFoundException("Could not find recipe: " + recipeNmae) : recipe;
    }
}
