...
public sealed class DeleteRecipeCommandValidator : AbstractValidator<DeleteRecipeCommand>
{
    ...
    {
        RuleFor(x => x.RecipeName).NotNull().NotEmpty().WithMessage("Recipe name is required!");
    }
}

public sealed class DeleteRecipeCommandHandler(IRecipeManagerRepository repo)
    : IRequestHandler<DeleteRecipeCommand, DeleteRecipeResult>
{
    public async Task<DeleteRecipeResult> Handle(
